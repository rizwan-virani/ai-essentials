...
(function () {
  ...
  var S = window.AIESS || {};
  var PASS = (S.exam && S.exam.passPercent) || 80;
  ...
  var Store = (function () {
    ...
    return {
      ...
      get: function (k, fb) { return read("aiess." + k, fb); },
      set: function (k, v) { write("aiess." + k, v); },
      del: function (k) { try { localStorage.removeItem("aiess." + k); } catch (e) {} }
    };
  })();
  ...
  function statCard() {
    ...
    mocks.forEach(function (m) { if ((m.pct || 0) > best) best = m.pct; });
  ...
  function renderDashboard() {
    ...
    b1.appendChild(el("p.sectsub", { text: "Start here. Understand how the AI Essentials CompCert is scored and delivered, and where practical AI fluency fits in your career." }));
    var g1 = el("div.grid");
    g1.appendChild(card({
      icon: "🎓", tag: "OVERVIEW", title: "Assessment Mechanics", cls: "",
      desc: "Format, timing, scoring, and question types for the AI Essentials CompCert &mdash; the details every candidate should internalize before sitting the assessment.",
      foot: "90-min mock · 75 items · " + PASS + "% mastery", cta: "Read",
      onClick: function () { openExamMechanics(); }
    }));
    g1.appendChild(card({
      icon: "🧭", tag: "CAREER", title: "Career Guidance", tagCls: "ghost",
      desc: "Where practical AI fluency fits in a modern career: the roles and everyday tasks AI Essentials strengthens across any field, technical or not.",
      foot: "Foundational · any field", cta: "Read",
    ...
    if (window.FLASH) {
      ...
      gf2.appendChild(card({
        ...
        desc: "Every AI Essentials term and acronym across all five domains in one deck &mdash; the highest-yield rote material.",
    ...
    var b6 = dashSection("06", "Performance-based question simulators", (S.pbqs ? S.pbqs.length : 30) + " simulations · 5 formats");
    b6.appendChild(el("p.sectsub", { html: "A database of <b>" + (S.pbqs ? S.pbqs.length : 30) + "</b> hands-on PBQs across five decision formats. Read the exhibit, complete each field, and submit for graded feedback mapped to the objective." }));
    var g4 = el("div.grid");
    ...
    var b5 = dashSection("08", "Mock certification exam", "75 questions · timed");
    b5.appendChild(el("p.sectsub", { html: "A realistic, full-length practice capstone for the AI Essentials CompCert &mdash; 75 questions weighted across the five study domains. Sit it under timed conditions to benchmark your readiness." }));
    var g3c = el("div.grid");
    g3c.appendChild(card({
      icon: "🧪", tag: "MOCK EXAM", title: "Full-Length Mock Exam",
      desc: "A realistic 90-minute practice run: 75 questions weighted across the five study domains, with a countdown timer, question flagging, and a domain-by-domain scoring dashboard reporting percentage mastery.",
      foot: "75 questions · 90 minutes · timed", cta: "Begin",
    ...
    g5.appendChild(card({
      ...
      desc: "Vetted, free study tools: the official CompTIA AI Essentials course page, free AI-literacy resources, and trusted community references.",
    ...
    g5.appendChild(card({
      ...
      desc: "Plan your certification journey: browse 42 industry certifications across 12 vendors with live filters, costs, full exam blueprints, DoD 8140 mapping, and salary and role insights.",
      foot: "42 certifications · 12 vendors", cta: "Explore",
  ...
  /* ===========================================================================
     ...
     =========================================================================== */
  function certVer() { var v = window.AIESS_VER || ""; return v ? "?v=" + v : ""; }
  ...
  function renderCertCatalog(mount) {
    ...
    var search = el("input.cert-search", { type: "text", placeholder: "e.g. AI Essentials" });
  ...
  function computeReadiness() {
    ...
    var bestMock = null; mocks.forEach(function (m) { if (m.pct != null && (bestMock == null || m.pct > bestMock)) bestMock = m.pct; });
    ...
    var dims = [
      ...
      { key: "mock", label: "Mock exam", weight: 1.6, value: bestMock != null ? bestMock : null, has: mocks.length > 0, kind: "score", detail: bestMock != null ? ("best " + bestMock + "%") : "not attempted" },
  ...
  function readinessOutcome(R) {
    if (R.bestMock != null) {
      var m = R.bestMock;
      if (m >= PASS + 8) return { cls: "good", text: "Strong mastery projected. Your best mock of " + m + "% clears the " + PASS + "% mastery target with margin." };
      if (m >= PASS) return { cls: "good", text: "On track. Your best mock of " + m + "% is at or above the " + PASS + "% mastery target. Keep it consistent." };
      if (m >= PASS - 10) return { cls: "warn", text: "Borderline. Your best mock of " + m + "% sits just under the " + PASS + "% mastery target. A focused push should get you there." };
      return { cls: "crit", text: "Not yet. Your best mock of " + m + "% is below the " + PASS + "% mastery target. Keep building before you sit the assessment." };
    }
  ...
  /* Lazily fetch a domain's reading module the first time it's opened, so the
     ...
     script populates AIESS.reading[id]; once cached it renders instantly. */
  ...
  function loadDomainModule(id, onReady, onFail) {
    ...
    s.src = "assets/js/content/domain" + id + ".js" + (window.AIESS_VER ? "?v=" + window.AIESS_VER : "");
    s.async = true;
    s.onload = function () {
  ...
  function openDomain(id) {
    ...
    function render() {
      ...
      Router.replace(readerView("Domain " + id, d.title, "AI Essentials Domain " + id + " · " + d.weight + "% of the course", sections));
    }
  ...
  function openExamMechanics() { Router.open(readerView("Assessment mechanics", "Assessment Mechanics: AI Essentials", "How the CompCert is built, delivered, and scored", S.examMechanics || [])); }
  function openCareer() { Router.open(readerView("Career guidance", "Career Guidance", "Where AI fluency fits in your path", S.careerGuidance || [])); }
    ...
    Router.open({
      tag: "Analytics", render: function (host) {
        ...
        h.slice(0, 60).forEach(function (r) {
          ...
          var pass = r.mode === "mock" ? r.pct >= PASS : r.pct >= 70;
          var scoreCell = el("span.pillscore." + (pass ? "pass" : "fail"), { text: r.pct + "%" });
