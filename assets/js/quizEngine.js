/* ai essentials :: quizEngine.js  (data bank + assessment engine) */
AIESS.questions = AIESS.questions || [];
AIESS.pbqs = AIESS.pbqs || [];

/* ============================================================================
   ai essentials  ::  quizEngine.js  —  ENGINE  (assessment + PBQ runtime)
   This block is loaded after the question bank (content/quiz1..5.js) and PBQ database (content/pbqs.js).
...
(function () {
  ...
  /* ===========================================================================
     ...
     Persists two things across every quiz/mock session (localStorage via APP):
       aiess.objstats.v1 = { "1.2": {c, n}, ... }   per-objective accuracy
       aiess.missed.v1   = { "D4-071": 1, ... }      questions answered wrong
  ...
  function showResults(r) {
    A.open({
      tag: "Results", render: function (host) {
        ...
        var pass = isMock ? r.pct >= ((S.exam && S.exam.passPercent) || 80) : r.pct >= 70;
        ...
        if (isMock) hero.appendChild(el("div.scaled", { html: r.pct + "% &nbsp;·&nbsp; Mastery target <b style='color:var(--ink)'>" + ((S.exam && S.exam.passPercent) || 80) + "%</b> <span class='dim'>— the AI Essentials CompCert has no official passing score; the real credential is a 30-minute online assessment</span>" }));
  ...
  function startMock() {
    ...
    function submit() {
      ...
      var pct = Math.round(correct / items.length * 100);
      QS.record(items, picks); // feed adaptive engine + missed-question queue
      A.store.record({ mode: "mock", modeLabel: "Mock exam", detail: correct + "/" + items.length + " · used " + Math.round((DURATION - remaining) / 60) + " min", pct: pct, correct: correct, total: items.length, domainStats: stats });
      showResults({ tag: "Results", title: "Mock Exam", mode: "mock", correct: correct, total: items.length, pct: pct, stats: stats, wrongList: wrongList, retry: startMock });
    }
  }
